...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하여 제목, 내용을 입력받고 작성에 성공하면 성공 처리를 합니다.
작성에 실패하면 실패 처리 됩니다. 인증, 인가 실패, 데이터베이스 에러, 데이터 유효성 검사 실패가 발생할 수 있습니다.
...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하여 요청을 보내면 기본적으로 작성일 기준 내림차순으로 게시물 리스트를 반환합니다.
...
만약 불러오기에 실패하면 실패처리를 합니다. 
존재하지 않는 게시물, 데이터 유효성 실패, 인가 실패, 데이터베이스 에러가 발생할 수 있습니다.
...
##### 설명
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하여 접수 번호와 답글 내용을 입력받고 요청을 보내면 해당하는 Q&A 게시물의 답글이 작성됩니다.
만약 답글 작성에 실패하면 실패처리를 합니다. 
인가 실패, 데이터베이스 에러가 발생할 수 있습니다.
...
##### 설명
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하여 접수 번호를 입력받고 요청을 보내면 해당하는 Q&A 게시물이 삭제됩니다.
만약 삭제에 실패하면 실패처리를 합니다. 
유효성 검사 실패, 존재하지 않는 게시물, 인가 실패, 데이터베이스 에러가 발생할 수 있습니다.
...
클라이언트로부터 Request Header의 Authorization 필드로 Bearer 토큰을 포함하여 접수번호, 제목, 내용을 입력받고 수정에 성공하면 성공 처리를 합니다.
수정에 실패하면 실패 처리 됩니다. 
존재하지 않는 게시물, 답변이 작성된 게시물, 인가 실패, 데이터베이스 에러, 데이터 유효성 검사 실패가 발생할 수 있습니다.
...
(매매가, 전세가, 월세 보증금 데이터의 단위는 천원 단위)
만약 불러오기에 실패하면 실패 처리 됩니다. 
